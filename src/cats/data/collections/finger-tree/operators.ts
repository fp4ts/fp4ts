...
import { Deep, Empty, Node, Single, view, FingerTree } from './algebra';
...
export const isEmpty: <A>(xs: FingerTree<A>) => boolean = xs => xs === Empty;
export const nonEmpty: <A>(xs: FingerTree<A>) => boolean = xs => xs !== Empty;

export const head: <A>(xs: FingerTree<A>) => A = xs =>
  headOption(xs).getOrElse(() => throwError(new Error('Vector.empty.head')));

export const headOption: <A>(xs: FingerTree<A>) => Option<A> = xs =>
  popHead(xs).map(([h]) => h);

export const tail: <A>(xs: FingerTree<A>) => FingerTree<A> = xs =>
  popHead(xs)
    .map(([, tl]) => tl)
    .getOrElse(() => empty);

export const last: <A>(xs: FingerTree<A>) => A = xs =>
  lastOption(xs).getOrElse(() => throwError(new Error('Vector.empty.last')));

export const lastOption: <A>(xs: FingerTree<A>) => Option<A> = xs =>
  popLast(xs).map(([h]) => h);

export const init: <A>(xs: FingerTree<A>) => FingerTree<A> = xs =>
  popLast(xs)
    .map(([, tl]) => tl)
    .getOrElse(() => empty);

export const toList = <A>(v: FingerTree<A>): List<A> =>
  foldRight_(v, List.empty as List<A>, (x, xs) => xs.prepend(x));

export const toArray = <A>(v: FingerTree<A>): A[] =>
  foldLeft_(v, [] as A[], (xs, x) => [...xs, x]);

export const popHead = <A>(v: FingerTree<A>): Option<[A, FingerTree<A>]> => {
  const ft = view(v);
...
export const popLast = <A>(v: FingerTree<A>): Option<[A, FingerTree<A>]> => {
  const ft = view(v);
...
export const prepend: <B>(
  x: B,
) => <A extends B>(xs: FingerTree<A>) => FingerTree<B> = x => xs =>
  prepend_(xs, x);
...
export const concat: <B>(
  ys: FingerTree<B>,
) => <A extends B>(xs: FingerTree<A>) => FingerTree<B> = ys => xs =>
  concat_(xs, ys);

export const foldLeft: <A, B>(
  z: B,
  f: (b: B, a: A) => B,
) => (xs: FingerTree<A>) => B = (z, f) => xs => foldLeft_(xs, z, f);

export const foldRight: <A, B>(
  z: B,
  f: (a: A, b: B) => B,
) => (xs: FingerTree<A>) => B = (z, f) => xs => foldRight_(xs, z, f);

// -- Point-ful operators

export const prepend_ = <A>(v: FingerTree<A>, x: A): FingerTree<A> => {
  const ft = view(v);
...
export const append_ = <A>(v: FingerTree<A>, x: A): FingerTree<A> => {
  const ft = view(v);
...
export const concat_ = <A>(
  xs: FingerTree<A>,
  ys: FingerTree<A>,
): FingerTree<A> => _concatWithMiddle(xs, [], ys);
