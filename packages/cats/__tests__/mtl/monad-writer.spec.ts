...
describe('MonadWriter', () => {
  checkAll(
    ...
    MonadWriterSuite(
      ...
      X => eq.fn1Eq(ec.miniInt(), Eval.Eq(Eq.tuple(X, Eq.primitive))),
    ),
  );
  ...
  checkAll(
    ...
    MonadWriterSuite(
      ...
      <X>(X: Eq<X>): Eq<OptionT<$<WriterTF, [EvalF, string]>, X>> =>
        OptionT.Eq(Eval.Eq(Eq.tuple(Option.Eq(X), Eq.primitive))),
    ),
  );
  ...
  checkAll(
    ...
    MonadWriterSuite(
      ...
      <X>(X: Eq<X>): Eq<EitherT<$<WriterTF, [EvalF, string]>, string, X>> =>
        EitherT.Eq(Eval.Eq(Eq.tuple(Either.Eq(Eq.primitive, X), Eq.primitive))),
    ),
  );
});
