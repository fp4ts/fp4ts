...
import { Ord, Eq, Monoid, Some, None, CommutativeMonoid } from '@fp4ts/cats';
import { Set, List } from '@fp4ts/collections-core';
import { isValid } from '@fp4ts/collections-core/lib/set/operators';
...
describe('set', () => {
  ...
  describe('tail', () => {
    ...
    it(
      ...
      forAll(CA.fp4tsSet(fc.integer(), Ord.fromUniversalCompare()), xs =>
  ...
  describe('insert', () => {
    ...
    it(
      'should remain valid tree',
      forAll(
        CA.fp4tsSet(fc.integer(), Ord.fromUniversalCompare()),
        fc.integer(),
  ...
  describe('union', () => {
    ...
    test(
      ...
      forAll(CA.fp4tsSet(fc.integer(), Ord.fromUniversalCompare()), s => {
        const r = s.union(s);
        return (
  ...
  describe('Tree validity', () => {
    ...
    const actionArbitrary = <A>(
      ...
      fc.oneof(
        ...
        CA.fp4tsSet(arbA, O).map(that => ({
          type: 'intersect' as const,
          that,
        })),
        CA.fp4tsSet(arbA, O).map(that => ({ type: 'union' as const, that })),
        CA.fp4tsSet(arbA, O).map(that => ({
  ...
  describe('Laws', () => {
    ...
    checkAll(
      ...
      FoldableSuite(Set.Foldable).foldable(
        ...
        Eq.fromUniversalEquals(),
        x => CA.fp4tsSet(x, Ord.fromUniversalCompare() as Ord<any>),
      ),
    );
  });
});
