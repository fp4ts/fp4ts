...
import { isValid } from '@fp4ts/collections-core/lib/map/operators';
...
describe('Map', () => {
  ...
  describe('tail', () => {
    ...
    it(
      'should remain valid',
      forAll(
        CA.fp4tsMap(fc.integer(), fc.string(), Ord.fromUniversalCompare()),
  ...
  describe('traverse', () => {
    it('should produce some when map contains only even values', () => {
      expect(
        Map([1, 2], [3, 4]).traverse(Option.Monad)(v =>
          v % 2 === 0 ? Some(v) : None,
        ),
    ...
    it('should produce none when contains odd values', () => {
      expect(
        Map([1, 2], [3, 5]).traverse(Option.Monad)(v =>
          v % 2 === 0 ? Some(v) : None,
        ),
      ).toEqual(None);
    });
  ...
  describe('Tree validity', () => {
    ...
    const actionArbitrary = <K, V>(
      ...
      fc.oneof(
        ...
        CA.fp4tsMap(arbK, arbV, O).map(that => ({
    ...
    it(
      ...
      forAll(
        CA.fp4tsMap(fc.integer(), fc.integer(), Ord.fromUniversalCompare()),
        fc.array(
          actionArbitrary(
            fc.integer(),
            fc.integer(),
  ...
  describe('Laws', () => {
    checkAll(
      ...
      MonoidKSuite(Map.MonoidK(Ord.fromUniversalCompare())).monoidK(
        ...
        x => CA.fp4tsMap(fc.integer(), x, Ord.fromUniversalCompare()),
